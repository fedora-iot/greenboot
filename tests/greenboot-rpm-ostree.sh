...
sudo dnf install -y --nogpgcheck httpd osbuild osbuild-composer composer-cli podman skopeo wget firewalld lorax xorriso curl jq expect qemu-img qemu-kvm libvirt-client libvirt-daemon-kvm libvirt-daemon virt-install rpmdevtools ansible-core createrepo_c
...
cp rpmbuild/RPMS/x86_64/*.rpm /var/www/html/packages/ && popd
...
# Test IoT/Edge OS
ansible-playbook -v -i "${TEMPDIR}/inventory" -e ostree_commit="${INSTALL_HASH}" greenboot-rpm-ostree.yaml || RESULTS=0
